...
/**
 * @ClassName DwsUserUserLoginWindowApp
 * @Package com.atguigu.gmall.realtime.dws.app
 * @Author CaiBW
 * @Create 24/02/18 下午 1:54
 * @Description
 */
@Slf4j
public class DwsUserUserLoginWindowApp extends BaseApp {
    ...
    public void handle(StreamExecutionEnvironment env, DataStreamSource<String> stream) {
        ...
        //2. 按照uid分组， 统计回流和独立用户
        SingleOutputStreamOperator<UserLoginBean> uuBackStream = uuBackStream(etlStream);
        
        //uuBackStream.print();
        
        //3. 分配水位线
        SingleOutputStreamOperator<UserLoginBean> withWatermarkStream = withWatermark(uuBackStream);
        
        //4. 开窗聚合
        SingleOutputStreamOperator<UserLoginBean> windowStream = windowAgg(withWatermarkStream);
    ...
    private static void writeToDoris(SingleOutputStreamOperator<UserLoginBean> windowStream) {
        windowStream.map(
    ...
    private static SingleOutputStreamOperator<UserLoginBean> windowAgg(SingleOutputStreamOperator<UserLoginBean> withWatermarkStream) {
    ...
    private static SingleOutputStreamOperator<UserLoginBean> withWatermark(SingleOutputStreamOperator<UserLoginBean> uuBackStream) {
    ...
    private static SingleOutputStreamOperator<UserLoginBean> uuBackStream(SingleOutputStreamOperator<JSONObject> etlStream) {
        SingleOutputStreamOperator<UserLoginBean> uuBackStream = etlStream.keyBy(
